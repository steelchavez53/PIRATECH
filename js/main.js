...
const price = document.getElementById("sliderPrice");
function animateLetters(element, text) {
  element.innerHTML = ''; // Limpiar texto anterior
  [...text].forEach((char, i) => {
...
function updateSlider() {
  ...
  images.forEach((img, i) => {
    ...
    } else if (i === (imageIndex + 1) % images.length) {
      img.classList.add('next');
    } else if (i === (imageIndex - 1 + images.length) % images.length) {
  ...
  backgrounds.forEach((bg, i) => {
    bg.style.opacity = (i === imageIndex) ? 1 : 0;
  });

  // Actualizar contenido de texto
  const activeImage = images[imageIndex];
animateLetters(title, activeImage.dataset.title);
animateLetters(description, activeImage.dataset.description);
price.textContent = activeImage.dataset.price; // glitch solo aplica
